...
//
// Works with either ANTHROPIC_API_KEY or OPENAI_API_KEY — whichever is set.
// If both are set, Anthropic is used. If neither is set, or the call fails
// for any reason, composeAIReading() returns null and the caller
...
// Picks whichever provider has a key configured. Anthropic wins if both are
// set — there's no meaningful quality reason for that, it's just a
// deterministic tie-break so behavior doesn't depend on load order.
  ...
  try {
    ...
    const text =
      provider.name === "anthropic"
        ? await callAnthropic(provider.client, userPrompt)
        : await callOpenAI(provider.client, userPrompt);
    return text || null;
  } catch (err) {
